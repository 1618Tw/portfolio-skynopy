...
function GalleryScene({
  ...
  const handleWheel = useCallback(
    (event: WheelEvent) => {
      // Don't preventDefault — let the page scroll past the hero normally.
      // The gallery still reacts to wheel velocity for its cloth effects.
      setScrollVelocity((prev) => prev + event.deltaY * 0.005 * speed);
      setAutoPlay(false);
    ...
    <>
      {planesData.current.map((plane, i) => {
        ...
        const img = texture.image as
          ...
          | undefined;
        const aspect = img && img.width > 0 ? img.width / img.height : 1;
